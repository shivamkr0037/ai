...
def handle_message(update, context):
    ...
    for part in response_parts:
        if part.startswith('data:{"content":"'):
            content = json.loads(part[5:])["content"].strip()  # Remove leading and trailing spaces
            if content:
                if previous_message:
            ...
            content = json.loads(part)["content"].strip()  # Remove leading and trailing spaces
            if content:
                sent_message = send_message(context.bot, update.effective_chat.id, content)
                message_id = sent_message.message_id
                time.sleep(1)  # Delay between messages to avoid flooding the API
