            ...
            if content:
                ...
                for i, message in enumerate(previous_messages):
                    if i == len(previous_messages) - 1:
                        sent_message = send_message(context.bot, update.effective_chat.id, message)
                        message_id = sent_message.message_id
                    else:
                        edit_message(context.bot, update.effective_chat.id, message_id - (len(previous_messages) - 1 - i), message)
                time.sleep(1) # Delay between edits to avoid flooding the API
